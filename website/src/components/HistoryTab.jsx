  ...
  return (
    <article className="max-w-2xl mx-auto py-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-gray-700 leading-relaxed font-normal">
      ...
      <section className="mb-10">
        ...
        <p className="text-[15px] sm:text-base leading-7 mb-3.5">
          Around 2021, managing custom GCP virtual machines was taking too much sysadmin effort for a small academic team, and cloud grant funding was not a permanent solution. At the same time, Google had recently rolled out <a href="https://colab.research.google.com/" target="_blank" rel="noopener noreferrer" className="text-tcd-blue font-medium hover:underline">Google Colab</a>. Beyond eliminating server hosting bills, we saw this as a great educational opportunity: working with Colab gave students hands-on experience with an accessible, industry-standard cloud tool that they could continue using in their own research, final-year projects, and careers after graduation.
        </p>
        <p className="text-[15px] sm:text-base leading-7 mb-3.5">
          Vibhoothi, who had recently joined our research group after contributing to <a href="https://www.videolan.org/" target="_blank" rel="noopener noreferrer" className="text-tcd-blue font-medium hover:underline">VideoLAN</a>, redesigned the student workflow around Colab GPU runtimes, pairing them with students' institutional Trinity Google Drive accounts for persistent storage. Because free Colab instances don't have an interactive terminal, Vibhoothi wrote a helper notebook (<code className="text-xs bg-slate-100 text-slate-700 px-1 py-0.5 rounded font-mono border border-slate-200">4c16-git-workflow.ipynb</code>) that runs Git commands (clone, commit, push) directly through the Python kernel to sync Drive storage with the server.
        </p>
        <p className="text-[15px] sm:text-base leading-7 mb-4">
          This solved the hardware barrier completely and allowed the class to expand from 70 to over 150 students across multiple degree streams. However, the backend still relied on the legacy Gitolite, Ruby on Rails, and synchronous <code className="text-xs bg-slate-100 text-slate-700 px-1 py-0.5 rounded font-mono border border-slate-200">post-receive</code> hooks (with <a href="https://caddyserver.com/" target="_blank" rel="noopener noreferrer" className="text-tcd-blue font-medium hover:underline">Caddy</a> added for HTTPS). Because grading was synchronous, each student's <code className="text-xs bg-slate-100 text-slate-700 px-1 py-0.5 rounded font-mono border border-slate-200">git push</code> had to block and hold the connection open while test scripts ran. With larger cohorts pushing simultaneously right before deadlines, requests queued up and led to frequent connection timeouts.
        </p>

        <h3 className="text-base font-semibold text-gray-900 mt-6 mb-2">
          Generation 3 (2026-Present): Platform Overhaul & Asynchronous Architecture on 4c16.dev
        </h3>
        <p className="text-[15px] sm:text-base leading-7 mb-3.5">
          To eliminate push contention and prepare for upcoming growth—with deep learning expanding into a new third-year undergraduate module in 2027/28—Vibhoothi architected a complete overhaul of the platform in 2026.
        </p>
        <p className="text-[15px] sm:text-base leading-7 mb-3.5">
          The fundamental breakthrough was moving to a fully <strong>asynchronous, event-driven model</strong>. Pushing code is now instantaneous: Forgejo receives the push and returns control immediately without blocking. It dispatches a webhook to FastAPI, which queues an evaluation task in PostgreSQL. Sandboxed Docker workers claim jobs and grade submissions in the background, writing results back to the database so students see live updates on their web dashboard in under ten seconds.
        </p>
        <p className="text-[15px] sm:text-base leading-7 mb-3.5">
          The entire stack runs as 5 <a href="https://www.docker.com/" target="_blank" rel="noopener noreferrer" className="text-tcd-blue font-medium hover:underline">Docker</a> containers on a single virtual host, replacing fragile command-line maintenance with an intuitive web administration interface:
        </p>
